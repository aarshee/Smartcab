	...
	if good_ratio == 1: # Perfect driving
		...
		if data['actions'].apply(lambda x: ast.literal_eval(x)[4]).sum() > 0: # Major accident
			...
			if minor >= len(data)/2: # Minor violation in at least half of the trials
				return (value[0],"B: Minor Violation", "green")
			else:
				return (value[0],"A: Minor Violation", "green")
...
def calculate_reliability(data):
	...
	if success_ratio == 1: # Always meets deadline
		return (success_ratio, "A+", "green")
	else:
		if success_ratio >= 0.90:
			return (success_ratio, "A", "green")
		elif success_ratio >= 0.80:
			return (success_ratio, "B", "green")
		elif success_ratio >= 0.70:
			return (success_ratio, "C", "#EEC700")
		elif success_ratio >= 0.60:
			return (success_ratio, "D", "#EEC700")
		else:
			return (success_ratio, "F", "red")
...
def plot_trials(csv):
	...
	if len(testing_data) > 0:
		safety_value, safety_rating, safety_color = calculate_safety(testing_data)
		reliability_value, reliability_rating, reliability_color = calculate_reliability(testing_data)

		# Write success rate
		ax.text(0.40, .9, "{} testing trials simulated.".format(len(testing_data)), fontsize=14, ha='center')
		ax.text(0.40, 0.7, "Safety Rating & Value:", fontsize=16, ha='center')
		ax.text(0.40, 0.42, "{}, {}".format(safety_rating, safety_value), fontsize=16, ha='center', color=safety_color)
		ax.text(0.40, 0.27, "Reliability Rating & Value:", fontsize=16, ha='center')
		ax.text(0.40, 0, "{}, {}".format(reliability_rating, reliability_value), fontsize=16, ha='center', color=reliability_color)
